...
class DiscretePolicy(nn.Module):
    ...
    def get_action_mask(self, states):
        # the last dim is time so exclude it
        time_steps = states[:, -1]
        waitlist = states[:, self.env.num_sessions:-1]
        ...
        for i, w in enumerate(waitlist):
            for j, action in enumerate(self.action_space):
                ...
                if torch.all(action <= w).item() and time_steps[i] < self.env.decision_epoch:
                    mask[i, j] = 1
                elif torch.all(action == w).item() and time_steps[i] == self.env.decision_epoch:
                    mask[i, j] = 1
        return mask
    ...
    def forward(self, states):
        ...
        action_prob = torch.softmax(self.action_head(x) + torch.log(action_masks + 1e-9), dim=1)
        return action_prob
...
if __name__ == '__main__':
    ...
    states = torch.tensor([
                                 [0, 0, 11, 1, 5]], dtype=torch.float32)
    ...
    print(actor.action_space[actor.get_action_mask(states)[0].bool()])
