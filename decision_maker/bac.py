...
class BAC:
    ...
    def train(self, init_state, init_t, epoch_num, max_time_steps, batch_size, replay_memory, svd_low_rank, state_coefficient, fisher_coefficient):
        ...
        total_rewards = []
        expect_total_reward = 0
        for e in tqdm(range(1, epoch_num + 1)):
            state, info = self.env.reset(init_state, init_t)
            # prev_state = running_state(prev_state)
            total_reward = 0
            for time_step in range(1, max_time_steps+1):
                action = self.policy(state, time_step, train=True)
                # observation, reward, terminated, truncated, info
                next_state, reward, done, truncated, info = self.env.step(action)
                reward *= -1
                total_reward += self.discount**(time_step-1)*reward
                # state = running_state(state)
                replay_memory.add(state, action, next_state, reward, done, time_step)
            ...
            total_rewards.append(expect_total_reward)
